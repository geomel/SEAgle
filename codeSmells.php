...
<script type="text/javascript">
	...
	//php to Javascript array Variables
		...
		var	godClassArray = new Array();
		var	dataClassArray = new Array();
		var	featureEnvyArray = new Array();
		
 function php2Js(){
	<?php
			
			foreach($_SESSION['godClass'] as $key=>$value){
				echo "godClassArray[".$key."]=".$value.";";
			}
			foreach($_SESSION['dataClass'] as $key=>$value){
				echo "dataClassArray[".$key."]=".$value.";";
			}
			foreach($_SESSION['featureEnvy'] as $key=>$value){
				echo "featureEnvyArray[".$key."]=".$value.";";
			}
 ...
 function drawLinePlot(flag, ydata){
			...
			var options = {
				...
				switch(flag) {
						case "1":
							var god_plot = $.plot($("#godchart"), [tableData], options);
							break;
						case "2":
							var data_plot = $.plot($("#datachart"), [tableData], options);
							break;
						case "3":
							var feature_plot = $.plot($("#featurechart"), [tableData], options);
							break;	
				}
...
$(document).ready(function() {
		...
		createJSTableDataForGraphs(godClassArray);
		addCharts("001","God Class", "godchart");
		drawLinePlot("1", "godClass");
		createJSTableDataForGraphs(dataClassArray);
		addCharts("002","Data Class", "datachart");
		drawLinePlot("2", "dataClass");
		createJSTableDataForGraphs(featureEnvyArray);
		addCharts("003","Feature Envy", "featurechart");
		drawLinePlot("3", "featureEnvy");
